...
$type = $organisateur = $address = "";
$type_err = $organisateur_err =$address_err= "";
...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty(trim($_POST["organisateur"]))){
        ...
        $sql = "SELECT id_o FROM organisateur WHERE organisateur = :organisateur";
    ...
    if(  empty($type_err) &&  empty($organisateur_err)){

        // Prepare an insert statement
        $sql = "INSERT INTO organisateur (type, address, organisateur) VALUES (:type, :address, :organisateur) ";
        if($stmt = $pdo->prepare($sql)){
            // Bind variables to the prepared statement as parameters
  ...
  <img src="image/dam5.jpg"  style="float: right" />
    <div class="wrapper">
        ...
        <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
            ...
            <div class="form-group">
			<label> <h2>Supprimer un organisateur :</h2></label></br>
				<a href="delete2.php">Si vous voullez supprimer un organisateur cliquer ici . </a>
            </div>
        </form>
	</div>
